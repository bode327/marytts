...
public class Preprocess extends InternalModule {
	...
	static {
		...
		symbols.put("#", "hashtag");
		symbols.put("/", "barra");
		symbols.put("%", "porcento");
		symbols.put("+", "mais");
		symbols.put("-", "menos");
		symbols.put("=", "igual");
		symbols.put(">", "maior que");
		symbols.put("<", "menor que");
		symbols.put("&", "e");
	}

	public Preprocess() {
		super("Preprocess", MaryDataType.TOKENS, MaryDataType.WORDS, new Locale("pt","BR"));
		this.rbnf = new RuleBasedNumberFormat(new Locale("pt","BR"), RuleBasedNumberFormat.SPELLOUT);
		this.cardinalRule = "%spellout-numbering";
		this.ordinalRule = getOrdinalRuleName(rbnf);
		this.yearRule = getYearRuleName(rbnf);
		this.df = DateFormat.getDateInstance(DateFormat.LONG, new Locale("pt","BR"));
		try {
	...
	protected void expand(Document doc) throws ParseException, IOException, MaryConfigurationException {
		...
		while ((t = (Element) tw.nextNode()) != null) {
			...
			// remove commas
			if (MaryDomUtils.tokenText(t).matches("[\\$|£|€|R$]?\\d+,[\\d,]+")) {
			...
			if (MaryDomUtils.tokenText(t).matches("(?i)" + ordinalPattern.pattern())) {
				...
				// first check lexicon
				if (MaryRuntimeUtils.checkLexicon("pt_BR", MaryDomUtils.tokenText(t)).length == 0) {
				...
				MaryDomUtils.setTokenText(t, "ponto");
				webEmailTemp = webEmailTemp.replaceFirst("\\.", "ponto");
	...
	protected String expandDuration(String duration) {
		...
		durMatcher.find();
		String hrs = expandNumber(Double.parseDouble(durMatcher.group(1))) + " horas ";
		String mins = expandNumber(Double.parseDouble(durMatcher.group(2))) + " minutos ";
		String secs = expandNumber(Double.parseDouble(durMatcher.group(3))) + " segundo ";
		String ms = "";
		if (durMatcher.group(4) != null) {
			ms = "e " + expandNumber(Double.parseDouble(durMatcher.group(5))) + " millisegundos ";
		} else {
			secs = "e " + secs;
		}
	...
	protected String expandDate(String date) throws ParseException {
		...
		Date humanDate = df.getPatternInstance("DD/MM/YYYY", new Locale("pt","BR")).parse(date);
	...
	protected String expandMoney(String money, String currency) {
		...
		case "$":
			if (Double.parseDouble(currencyMatch.group(1)) > 1) {
				money = expandNumber(Double.parseDouble(currencyMatch.group(1))) + " dolars";
			} else {
				money = expandNumber(Double.parseDouble(currencyMatch.group(1))) + " dolar";
			}
			if (currencyMatch.group(2) != null) {
				int dotIndex = origText.indexOf('.');
				money = money + " " + expandNumber(Double.parseDouble(origText.substring(dotIndex + 1))) + " centavos";
			}
			break;
		case "£":
			money = expandNumber(Double.parseDouble(currencyMatch.group(1))) + " poudes estilinas";
			if (currencyMatch.group(2) != null) {
				int dotIndex = origText.indexOf('.');
		...
		case "R$":
			if (Double.parseDouble(currencyMatch.group(1)) > 1) {
				money = expandNumber(Double.parseDouble(currencyMatch.group(1))) + " reais";
			} else {
				money = expandNumber(Double.parseDouble(currencyMatch.group(1))) + " real";
			}
			if (currencyMatch.group(2) != null) {
				int dotIndex = origText.indexOf('.');
				money = money + " " + expandNumber(Double.parseDouble(origText.substring(dotIndex + 1))) + " centavos";
